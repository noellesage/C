...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    if(cur == NULL)
		...
		if(compare(cur->val, val) == -1) //curr less than val, go right
		{
			cur->right = _addNode(cur->right, val);
		}
		else if(compare(cur->val, val) == 1 || compare(cur->val, val)== 0 ) //curr equal to or less than val, go left
		{
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	struct Node * my_node = tree->root;
	temp->val = val;
	while(1) {
		if(compare(my_node->val, temp->val) == 1) {
		...
		else if(compare(my_node->val, temp->val) == -1) {
		...
		else if(compare(my_node->val, temp->val) == 0) {
			return 1;
		}
	}
  return 0;
